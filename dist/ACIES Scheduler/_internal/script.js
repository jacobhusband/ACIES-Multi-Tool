...
function initEventListeners() {
    ...
    document.getElementById('toolCleanDwgs').addEventListener('click', (e) => {
        ...
        document.getElementById('cleanDwgs_cadFiles').value = '';
    ...
    // Listeners for the new Clean DWGs dialog
    document.getElementById('btnSelectTitleblock').addEventListener('click', async () => {
        try {
            ...
            if (result.status === 'success' && result.paths.length > 0) {
                document.getElementById('cleanDwgs_titleblockPath').value = result.paths[0];
            }
        } catch (e) {
    ...
    document.getElementById('btnSelectCadFiles').addEventListener('click', async () => {
        try {
            const result = await window.pywebview.api.select_files({
                allow_multiple: true,
                file_types: ['AutoCAD Drawings (*.dwg)']
            ...
            if (result.status === 'success' && result.paths.length > 0) {
                document.getElementById('cleanDwgs_cadFiles').value = result.paths.join('\n');
            }
        } catch (e) {
            toast(`⚠️ Error selecting files: ${e.message}`);
        }
    });
    ...
    document.getElementById('btnProceedCleanDwgs').addEventListener('click', async () => {
        ...
        const cadFilesRaw = document.getElementById('cleanDwgs_cadFiles').value;
        const size = document.querySelector('input[name="cleanDwgs_size_radio"]:checked').value;

        if (!titleblockPath) {
            return toast('Please select a titleblock DWG.');
        }
        if (!cadFilesRaw) {
            return toast('Please select at least one CAD DWG to clean.');
        }

        const cadFilePaths = cadFilesRaw.split('\n').filter(p => p.trim() !== '');
