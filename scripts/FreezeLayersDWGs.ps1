...
$lblFreeze.Location = New-Object System.Drawing.Point(548, 74)
$lblFreeze.Size = New-Object System.Drawing.Size(370, 18)
...
$listToFreeze.Location = New-Object System.Drawing.Point(548, 96)
$listToFreeze.Size = New-Object System.Drawing.Size(370, 470)
$listToFreeze.Anchor = [System.Windows.Forms.AnchorStyles]::Top -bor [System.Windows.Forms.AnchorStyles]::Bottom -bor [System.Windows.Forms.AnchorStyles]::Left -bor [System.Windows.Forms.AnchorStyles]::Right
$listToFreeze.SelectionMode = "MultiExtended"
$listToFreeze.IntegralHeight = $false
$form.Controls.Add($listToFreeze)

$btnOk = New-Object System.Windows.Forms.Button
$btnOk.Text = "Freeze Layers on Right"
...
function Update-MoveButtons {
  ...
  $btnRemoveSelected.Enabled = $listToFreeze.SelectedItems.Count -gt 0
  $btnRemoveAll.Enabled = $listToFreeze.Items.Count -gt 0
}
...
function Refresh-LayerLists {
  ...
  $availableLayers = @($allSorted | Where-Object { -not $layersToFreezeSet.Contains($_) })
  ...
  $lbl.Text = "Move layers to the right list to freeze them."
  $lblAvailable.Text = "Available layers: $($availableLayers.Count)"
  $lblFreeze.Text = "Layers to freeze: $($layersToFreezeSorted.Count)"
  $btnOk.Enabled = $layersToFreezeSorted.Count -gt 0
  Update-MoveButtons
}
...
function Add-SelectedAvailableLayers {
  ...
  foreach ($layerName in $selected) { [void]$layersToFreezeSet.Add($layerName) }
...
function Add-AllVisibleAvailableLayers {
  ...
  foreach ($layerName in $visible) { [void]$layersToFreezeSet.Add($layerName) }
  Refresh-LayerLists $txtFilter.Text
  $listAvailable.Focus()
}
...
$btnRemoveSelected.add_Click({ Remove-SelectedFreezeLayers })
$btnRemoveAll.add_Click({ Remove-AllFreezeLayers })
...
if ($layersToFreeze.Count -eq 0) { Write-Host "No layers added to the right list. Exiting."; exit }
